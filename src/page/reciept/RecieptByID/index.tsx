...
import { GetRecieptById } from "../../../services/https/Reciept";
...
import { ArrowUturnLeftIcon, ExclamationTriangleIcon, PrinterIcon } from "@heroicons/react/24/outline";
import './re.css';
function RecieptProductByID() {
    ...
    const getReciept = async () => {
        let res = await GetRecieptById(Number(id));
        if(res) {setReciept(res);setInfo(res[0]);}}
    // const getReciept = async () => {
    //     let res = await GetRecieptById(Number(id));
    //     if(res) {setReciept(res);setInfo(res[0]);}}
    useEffect(()=>{
    ...
    return(
        <div className="flex justify-between items-center p-6 containerStyle " >
            <div className=" w-28 h-28 rounded-full p-3 text-[#22668D] transform transition-transform hover:scale-110 hover:text-[#144F71]"><div className="rounded-full border-2  drop-shadow-md bg-white p-3"><Link to="/reciepts"><ArrowUturnLeftIcon/></Link></div></div>
...
export default RecieptProductByID