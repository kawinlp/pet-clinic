...
import { DeletePetByID, GetPets } from "../../../services/https/petRegister";
...
const Index = () => {
  ...
  const showModal = (val: PetInterface) => {
    setModalText(`คุณต้องการลบข้อมูลสัตว์เลี้ยง "${val.Name} " หรือไม่ ?`);
    setDeleteId(val.ID);
    setOpen(true);
  };
                  ...
                  <table className="w-100% w-full min-w-[1000px] my-0 align-middle text-dark border-neutral-200">
                    ...
                    <thead>
                      <tr className="bg-gray-200 text-md font-semibold">
                        ...
                        <th className="text-center px-5 py-2">ประวัติการรักษา</th>
                      ...
                      {pets.map((pet, index) => (
                        <tr key={index} className="border-b border-[#22668D]">
                          ...
                          <td className=" text-start break-all py-2">
                            <img
                              src={pet.Picture}
                              className="w-[80px] h-[80px] rounded-[15px] align-middle object-cover"
                              alt="petImage"
                            />
                          </td>
                          <td className=" text-start break-all px-5 py-2">
                            {pet.Name}
                          </td>
                          <td className=" text-start break-all px-5 py-2">
                            {pet.Member?.FristName}
                          </td>
                          <td className=" text-center break-all px-5 py-2">
                            {pet.PetType?.Name}
                          </td>
                          ...
                          <td className=" text-center break-all px-5 py-2">
                            <Button
                              onClick={() =>
                                navigate(`/treatmentByPet/${pet.ID}`)
                              }
                              style={{ marginLeft: 8 }}
                              shape="circle"
                              icon={<EyeOutlined />}
